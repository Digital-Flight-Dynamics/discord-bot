...
async function renderAppealsDatabase(): Promise<void> {
    const user = await loadUser();
    moderationShell(user, 'appeals', 'Appeals DB', 'Search and manage every submitted appeal.', `<section class="database-search"><form class="mod-search-form" id="appeals-search"><input id="appeals-query" type="search" placeholder="Appeal ID, action ID, or subject" autocomplete="off" /><div class="custom-select appeals-status-filter" data-custom-select><input type="hidden" id="appeals-status" value="" /><button class="custom-select-trigger" type="button" aria-haspopup="listbox" aria-expanded="false" aria-label="Filter by appeal status"><i class="appeal-status-option-icon" data-lucide="eye"></i><span>All statuses</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m5 7.5 5 5 5-5"></path></svg></button><div class="custom-select-menu" role="listbox" hidden><button type="button" role="option" data-value="" aria-selected="true"><i class="appeal-status-option-icon" data-lucide="eye"></i><span>All statuses</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 10 4 4 8-8"></path></svg></button><button type="button" role="option" data-value="submitted" aria-selected="false"><i class="appeal-status-option-icon" data-lucide="clipboard-list"></i><span>Submitted</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 10 4 4 8-8"></path></svg></button><button type="button" role="option" data-value="review" aria-selected="false"><i class="appeal-status-option-icon" data-lucide="hourglass"></i><span>Under Review</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 10 4 4 8-8"></path></svg></button><button type="button" role="option" data-value="approved" aria-selected="false"><i class="appeal-status-option-icon" data-lucide="circle-check"></i><span>Approved</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 10 4 4 8-8"></path></svg></button><button type="button" role="option" data-value="denied" aria-selected="false"><i class="appeal-status-option-icon" data-lucide="circle-x"></i><span>Denied</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 10 4 4 8-8"></path></svg></button></div></div><button class="button" type="submit">Search database</button></form></section><section class="mod-panel" id="appeals-results"><div class="loading-inline"><span class="spinner"></span>Loading appeals…</div></section>`);
    const load = async (page = 1, limit: 10 | 25 = 10) => {
        const query = document.querySelector<HTMLInputElement>('#appeals-query')!.value.trim();
        const status = document.querySelector<HTMLInputElement>('#appeals-status')!.value;
    ...
    const appealsSearch = document.querySelector<HTMLFormElement>('#appeals-search');
    if (appealsSearch) initializeCustomSelects(appealsSearch);
    await load();
}
